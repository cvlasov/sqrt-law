...
function juniward_test(cover_fea_mat, stego_fea_mat, roc_curve_filename)
...
roc_file = fopen(roc_curve_filename,'w');
ROC_points = [Results.PFA; 1-Results.PMD];  % 5000x2 matrix
fprintf(roc_file, '%d %d\n', ROC_points);
[~ , Ind] = min((Results.PFA + Results.PMD)/2);
fprintf(roc_file, '%d\n', Ind);  % Write index of minPE on the last line
